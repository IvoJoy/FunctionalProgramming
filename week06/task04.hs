...
getAverageBalance (as, ps) p = roundD $ sum xs / (fromIntegral $ length xs)
 where
    xs = [money | (_, idAcc, money) <- as, personID@(idPer, _, _) <- ps, p personID, idAcc == idPer]

averageBalanceOfCities :: ([Account],[Person]) -> [String] -> Double
averageBalanceOfCities accper xs = getAverageBalance accper (\ (_, _, city) -> elem city xs)
