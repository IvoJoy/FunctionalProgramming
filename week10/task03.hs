...
distanceBetweenPoints (ThreeD x y z) (ThreeD p q r) = (sqrt((p - x)^2 + (q - y)^2 + (r - z)^2), (ThreeD x y z), (ThreeD p q r)) 
distanceBetweenPoints (TwoD x y) (TwoD m n) = (sqrt $ (m - x)^2 + (n - y)^2, (TwoD x y), (TwoD m n))

getDistance :: (Floating a) => [Point a] -> [Vector a]
getDistance xs = [distanceBetweenPoints x y | [x, y] <- subsequences xs]

getClosestDistance :: (Floating a, Ord a) => [Point a] -> Vector a
getClosestDistance xs = foldl1 (\ d@(distance, _, _) nd@(newDistance, _, _) -> if distance < newDistance then d else nd) (getDistance xs)