...
coldestTemp :: Country -> AvgYearlyTemperature
coldestTemp (Country _ capital cities) = minimum [temp | (City name _ temp) <- cities, name == capital]

vectorWithCountry :: [Country] -> [(Name, AvgYearlyTemperature)]
vectorWithCountry = map (\ c@(Country name _ _) -> (name, coldestTemp c)) 
