...
getNameLengthColor (movies, _, _) (f, g) = getTitleAndLength $ predMovies  
 where  
    predMovies = [movie | movie@(_, _, x, y, _) <- movies, f y, g x, x > (-1) && x/= longestColor]  
    getTitleAndLength = map (\(title, _, length, _, _) -> (title, length))  
    longestColor = foldr (\ (_, _, len, inC, _) trueLen -> if len > trueLen && inC == 'Y' then len else trueLen) 0 movies

getStudios :: MovieDB -> ([Year] -> [(StudioName, Name)])
getStudios db = (\ys -> map (\(x, y, _) ->(x, y)) $ filter (\(_, _, years) -> null years || any (`elem` years) ys) $ getStudiosAndProducers db) 

getStudiosAndProducers :: MovieDB -> [(StudioName, Name, [Year])]
getStudiosAndProducers (movies, studios, movieExecs) = filterStudios $ [(name, getProducer id, getYears name) | (name,id) <- studios]
 where
    getYears name = nub [year | (_, year, _, _, studio) <- movies, studio == name]
    getProducer id = head $ map (\(name, _, _) -> name) $ filter (\(_, pID, _) -> pID == id) movieExecs
...
filterStudios :: [(StudioName, Name, [Year])] -> [(StudioName, Name, [Year])]
filterStudios = filter (\(x, y, zs) -> allSame zs)
